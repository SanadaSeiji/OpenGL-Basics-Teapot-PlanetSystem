...
void display(){
	...
	//correct way to transform: first rotate, then translate
	...
	if (key_x_pressed == true) {
		...
		local1T = translate(local1T, vec3(0.01f, 0.0, 0.0));
	}
   ...
   mat4 global1 = local1;
	...
	local2 = scale(local2, vec3(0.5f, 0.5f, 0.5f));
	...
	//that's what make one child and one parent
	...
	local3 = scale(local3, vec3(0.5f, 0.5f, 0.5f));
	//child2 rotates around child1
	local3 = rotate_x_deg(local2, rotatez);
	// translation is 15 units in the y direction from the parents coordinate system
	//local3 = translate(local3, vec3(15.0, -10.0, 0.0));
	// global of the child is got by pre-multiplying the local of the child by the global of the parent
	...
	local4 = scale(local4, vec3(0.5f, 0.5f, 0.5f));
	//local4 = rotate_y_deg(local4, rotatez);
	//child3 rotates based on child2's position
	local4 = rotate_y_deg(local3, rotatez);
	// translation is 15 units in the y direction from the parents coordinate system
	//local4 = translate(local4, vec3(-15.0, 10.0, 0.0));
	// global of the child is got by pre-multiplying the local of the child by the global of the parent
	...
	local5 = scale(local5, vec3(0.5f, 0.5f, 0.5f));
	//child4 rotates based on child3's position
	local5 = rotate_z_deg(local4, rotatez);
	// translation is 15 units in the y direction from the parents coordinate system
	//local5 = translate(local5, vec3(15.0, 10.0, 0.0));
	// global of the child is got by pre-multiplying the local of the child by the global of the parent
...
void updateScene() {	
	...
	//unlike you used a unit_r, here just use 0.2f as unit
	rotatez+=0.01f;
	// Draw the next frame
	glutPostRedisplay();
}
